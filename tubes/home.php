...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function renderDots() {
        ...
        dotsBox.innerHTML = "";
        for (let i = 0; i < list.length; i++) {
            dotsBox.innerHTML += `<span class="dot ${i === index ? 'active-dot' : ''}" data-i="${i}"></span>`;
        }
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    searchInput.addEventListener("input", function () {
        ...
        let foundNow = false;
        let foundComing = false;

        // ===== NOW SHOWING =====
        document.querySelectorAll("#movieSlider .slide-card").forEach(card => {
            const title = card.querySelector(".slide-title").innerText.toLowerCase();
            const match = title.includes(keyword);

            card.style.display = match ? "block" : "none";
            if (match) foundNow = true;
        });

        // ===== COMING SOON =====
        document.querySelectorAll("#comingGrid .coming-card").forEach(card => {
            const title = card.querySelector("h3").innerText.toLowerCase();
            const match = title.includes(keyword);

            card.style.display = match ? "block" : "none";
            if (match) foundComing = true;
        });
